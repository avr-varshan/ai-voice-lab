...
S3_BUCKET = os.getenv("S3_BUCKET", "elevenlabs-clone-jas")
...
async def lifespan(app: FastAPI):
    global synthesiser, reference_style
...
TARGET_VOICES = {
    "Jasmine": "Models/LibriTTS/jas.wav",
...
async def generate_speech(request: TextOnlyRequest, background_tasks: BackgroundTasks):
    ...
    if request.target_voice not in TARGET_VOICES:
        raise HTTPException(
            status_code=400,
            detail=f"Target voice not supported. Choose from: {', '.join(TARGET_VOICES.keys())}"
        )
    try:
        ref_audio_path = TARGET_VOICES[request.target_voice]
        current_style = synthesiser.compute_style(ref_audio_path)
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            audio_chunk = synthesiser.inference(text=chunk, ref_s=current_style)
